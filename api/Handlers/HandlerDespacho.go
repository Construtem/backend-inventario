...
func RecibirFichaDespachoHandler(db *gorm.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		// Obtener id de URL y validar
		idStr := c.Param("id")
		id, err := strconv.Atoi(idStr)
		if err != nil {
		...
		// Bind JSON
		var req modelos.DespachoSimpleRequest
		if err := c.ShouldBindJSON(&req); err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"error": "JSON inválido"})
			return
		}

		// Validar que id URL y cotizacion_id coincidan
		if req.CotizacionID != id {
			c.JSON(http.StatusBadRequest, gin.H{"error": "CotizacionID no coincide"})
			return
		}

		// Llamar al controller
		despacho, err := Controllers.GuardarActualizarDespacho(db, req)
		if err != nil {

			c.JSON(http.StatusInternalServerError, gin.H{"error": "Error al guardar despacho"})
			return
		}

		c.JSON(http.StatusOK, gin.H{
			"message": "Ficha de despacho guardada correctamente",
			"data":    despacho,
		})
	}
}
